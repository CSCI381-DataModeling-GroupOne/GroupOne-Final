...
CREATE OR REPLACE FUNCTION log_ManufacturerVehicleMake_update() RETURNS TRIGGER AS $ManufacturerVehicleMake_audit$
    DECLARE
    ...
    BEGIN
        ...
        ELSIF (TG_OP = 'UPDATE') THEN
            ...
            SET ManufacturerVehicleMakeID = NEW.ManufacturerVehicleMakeID
        ...
        END IF;
    END;
$ManufacturerVehicleMake_audit$ LANGUAGE plpgsql;


-- trigger function logs updates to Country table, but only if something changed
CREATE TRIGGER uTdu_ManufacturerVehicleMake
    INSTEAD OF DELETE OR UPDATE ON Inventory.ManufacturerVehicleMake
    FOR EACH ROW
